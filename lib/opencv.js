...
cv.phash = function(im, hash_sideLen) { // can only support 3channels jpg, or grayscale pic.
  const side = hash_sideLen;
  let newImg = im; 
  if (im.channels() != 1) {
    im.convertGrayscale();
  }
  if (im.width() !== side || im.height !== side) {
    im.resize(side * 4, side * 4);
  }
  //im.resize( width, height );
  let floatInputMatrix = new cv.Matrix();
  im.convertTo(floatInputMatrix, cv.Constants.CV_32F);
  let srcDCT = floatInputMatrix.dct();

  let resultDCT = new cv.Matrix(side * 4, side * 4, cv.Constants.CV_32F, [0]);
  let zeroImg = new cv.Matrix(side * 4, side * 4, cv.Constants.CV_32F, [0]);
  resultDCT.absDiff(srcDCT, zeroImg);
  ...
  let average = sum/(side * side);
  let retStr = '';
  for (let i = 0; i < side; i++) {
    for (let j = 0; j < side; j++) {
      retStr += srcDCT.get(i,j) > average ? '1':'0';
    }
  }
  return retStr;
};